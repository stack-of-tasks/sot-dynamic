 ...
 * v_dot =  M^-1 (f - Bv)
 */
Vector& IntegratorForce::
computeDerivative( Vector& res,
		   const int& time )
{
  sotDEBUGIN(15);
...
Vector& IntegratorForce::
computeIntegral( Vector& res,
		 const int& time )
{
  sotDEBUGIN(15);

  const Vector & dvel =velocityDerivativeSOUT( time );
  res = dvel;
  res *= timeStep;
  if( velocityPrecSIN )
    {
...
Matrix& IntegratorForce::
computeMassInverse( Matrix& res,
		    const int& time )
{
  sotDEBUGIN(15);

  const Matrix & mass = massSIN( time );
  res = mass.inverse();
