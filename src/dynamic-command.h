...
namespace dynamicgraph { namespace sot {
  namespace command {
	...
	Matrix position = values[2].value();
	...
	std::vector<Value> values = getParameterValues();
	std::string jointName = values[0].value();
	Matrix inertiaMatrix = values[1].value();
	...
	Vector center = values[1].value();
	Vector thumbAxis = values[2].value();
	Vector forefingerAxis = values[3].value();
	Vector palmNormalAxis = values[4].value();
	...
	Vector anklePosition = values[3].value();
	...
	Dynamic& robot = static_cast<Dynamic&>(owner());
	std::vector<Value> values = getParameterValues();
	Vector gazeOrigin  = values[0].value();
	Vector gazeDirection = values[1].value();
